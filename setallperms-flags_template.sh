...
if [[ "$flag1" = "verbose" && "$flag2" = "silent" && "$flag3" = "on" ]] ||  [[ "$flag1" = "verbose" && "$flag2" = "default" && "$flag3" = "on" ]]; then
	# verbose logging on
	echo "" 2>&1 | tee -a /path/to/log.log
	command statement 2>&1 | tee -a /path/to/log.log
...
# if  verboseMENU is set(on) and verboseFLAGS is on and logging is on] or verboseMENU is default(on) and verboseFLAGS is on and logging is on]
	...
	# verbose logging on
	echo "ANNOUCEMENT:		" 2>&1 | tee -a /path/to/log.log
	echo "COMMAND:		" 2>&1 | tee -a /path/to/log.log
	command statement -v 2>&1 | tee -a /path/to/log.log
...
# if  [verboseMENU is set(off) and verboseFLAGS is on and logging is on]
	...
	#verbose logging on
	echo "ANNOUCEMENT:		" >> /path/to/log.log
	echo "COMMAND:		" >> /path/to/log.log
	command statement -v >> /path/to/log.log
